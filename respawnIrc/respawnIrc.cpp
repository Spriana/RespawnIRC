...
respawnIrcClass::respawnIrcClass(QWidget* parent) : QWidget(parent)
{
    ...
    /* Les deux sons sont chargés une fois pour toutes ici : QSoundEffect lit le fichier de façon
     * asynchrone, donc poser la source au moment de jouer ferait rater le premier bip. Les fichiers
     * sont livrés à côté du programme et jamais écrits, dataDirPath() est donc le bon chemin. */
...
void respawnIrcClass::showAbout()
{
    ...
    QMessageBox::information(this, "A propos de RespawnIRC", "<b>RespawnIRC version " + versionName + ".</b><br /><br />" +
                           "Ce logiciel a été developpé à l'aide de Qt 5 ainsi que de Hunspell <a style=\"color: " + styleTool::getColorInfo().linkColor + ";\" href=\"http://hunspell.github.io/\">http://hunspell.github.io/</a>.<br />" +
