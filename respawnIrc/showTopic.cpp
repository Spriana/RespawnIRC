...
bool showTopicClass::askIfThisMessageHasToBeDeleted(const QString& rawMessage)
{
    ...
    /* setButtonText est dépréciée par Qt 6, qui renvoie vers addButton. On passe plutôt par button(),
     * qui garde les boutons standards tels quels : addButton donnerait des boutons personnalisés, et
     * l'exec() du bas ne rendrait plus QMessageBox::Yes. */
